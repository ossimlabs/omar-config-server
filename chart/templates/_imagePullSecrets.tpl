...
*/}}
{{- define "omar-config-server.imagePullSecrets" -}}
  {{/*
  ...
  */}}
  ...
  {{- if .Values.generatedImagePullSecret -}}
    {{- if .Values.generatedImagePullSecret.create -}}
      {{- $secrets = (append $secrets (.Values.generatedImagePullSecret.name | default (include "omar-config-server.fullname" .))) }}
    {{- end -}}
  {{- end -}}
...
{{- define "omar-config-server.generateImagePullSecret" }}
